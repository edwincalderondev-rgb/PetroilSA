...
## Chat AIRA (2026-09-16) — hay DOS versiones conviviendo

**`index.html` usa el chat nuevo**; todas las demás páginas siguen con la maqueta vieja hasta que se apruebe la migración. No mezcles los dos.

| | Nuevo (solo index) | Viejo (resto del sitio) |
|---|---|---|
| CSS | `assets/css/aira.css` | `assets/css/chatbot.css` |
| JS | `assets/js/aira-kb.js` + `assets/js/aira.js` | `assets/js/chatbot.js` |
| Clases | `.aira-*` dentro de `#aira` | `.chat-*` |

Los prefijos son distintos **a propósito**: `ala-de-sable.css` sobrescribe `.chat-launcher`, y `.toast` (que usa `pqrsf.js`) vive dentro de `chatbot.css`. Con namespaces separados, migrar una página es cambiar 3 líneas sin romper las otras. Para migrar una página: copia el bloque `<div id="aira">` de index.html, pon `data-base="../"` (prefijo de los enlaces de la base de conocimiento) y ajusta las rutas de `<img>`/`<link>`/`<script>`. **Si la página usa `.toast` (`legal/pqrsf.html`), sigue necesitando `chatbot.css` o hay que mover esa regla.**
